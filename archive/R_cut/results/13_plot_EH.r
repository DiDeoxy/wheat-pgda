...
# # calc variance of each marker's eh value in its 10 closest upstream and
# # downstream markers
# wheat_data$snp <- by(wheat_data$snp, wheat_data$snp$chrom, 
#   function (chrom) {
#     vars <- vector(length = nrow(chrom))
#     for (i in 1:nrow(chrom)) {
#       if (i <= 2) {
#         vars[i] <- sd(chrom$eh[1:(i + 2)])
#       } else if (i >= (nrow(chrom) - 2)) {
#         vars[i] <- sd(chrom$eh[(i - 2):nrow(chrom)])
#       } else {
#         vars[i] <- sd(chrom$eh[(i - 2):(i + 2)])
#       }
#     }
#     vars
#   }
# ) %>% unlist(use.names = FALSE) %>%
#   cbind(wheat_data$snp, var_eh = .)

# identify those snps within the extended haplotypes
snp_index_1A <- which(wheat_data$snp$chrom == "1A")
...
snp_index_2A <- which(wheat_data$snp$chrom == "2A")
...
snp_index_4A <- which(wheat_data$snp$chrom == "4A")
...
snp_index_5B <- which(wheat_data$snp$chrom == "5B")
...
snp_index_6A <- which(wheat_data$snp$chrom == "6A")
...
snp_index_6B <- which(wheat_data$snp$chrom == "6B")
...
snp_index_7A <- which(wheat_data$snp$chrom == "7A")
...
plots <- by(wheat_data$snp, wheat_data$snp$chrom,
  function (data_chrom) {
    ...
    data_chrom %>%
      ...
      geom_smooth(
        aes(pos_mb, eh), colour = "black", size = 0.5
      )
  }
)
