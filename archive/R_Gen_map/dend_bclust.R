...
## loading the data
load("C:\\Users\\Max_H\\OneDrive - University of Guelph\\Pedagogy\\PGDA\\Data\\wheat_bclust.Rdata")
wheat.bclust <- as.dendrogram(wheat.bclust)
sample.order <- as.numeric(labels(wheat.bclust))
...
png("C:\\Users\\Max_H\\OneDrive - University of Guelph\\Pedagogy\\PGDA\\Figures\\dend\\bclust_compact.png", 
...
circos.track(ylim = c(0, 1), panel.fun = function(x, y) {
  circos.text(1:359, rep(0, 359), sample.id[sample.order],
...
circos.track(ylim = c(0, 1), panel.fun = function(x, y) {
  circos.rect(0.05:358.05, rep(0,359), 0.95:358.95, rep(1,359), border = colors()[468], lwd = 0.6,
              col = sapply(as.numeric(as.character(pop.code))[sample.order], function(x) { return(colours[x]) } ))
...
circos.track(ylim = c(0, 1), panel.fun = function(x, y) {
  circos.rect(0.05:358.05, rep(0,359), 0.95:358.95, rep(1,359), border = colors()[142], lwd = 0.6,
              col = sapply(as.numeric(pop.code)[sample.order], function(x) { return(colours[x]) } ))
}, track.height = 0.04, bg.border = NA)
...
palette(colours)
dend_colour <- color_branches(wheat.bclust, k = 7, col = colours)
