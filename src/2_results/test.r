...
wheat_data$snp[
  which(
    ...
    # & wheat_data$snp$pos_mb > 447
    # & wheat_data$snp$pos_mb < 727
  ),
] %>%
  rowwise() %>%
  mutate(type = 
    ifelse(chrw >= 0.5 && csws >= 0.5, "none",
      ifelse(chrw >= 0.5 && csws <= 0.5, "chrs_chrw_vs_csws",
        ifelse(chrw <= 0.5 && csws >= 0.5, "chrs_csws_vs_chrw", 
          ifelse(chrw <= 0.5 && csws <= 0.5, "chrs_vs_chrw_csws", NA)
        )
      )
    )
  ) %>%
