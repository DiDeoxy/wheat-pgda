...
results <- list()
for (grouping in c("desig", "era", "bp", "texture", "colour", "habit", 
                   "habit/texture_colour", "colour/texture_habit",
                   "texture/colour_habit", "hrs_other", "cluster",
                   "chrs_other")) {
  print(grouping)
  amova_result <- poppr.amova(full_genind, hier = as.formula(paste0("~", 
                                                             grouping)),
  ...
  if (grepl("/", grouping)) {
    ...
    amova_randtest <- with(amova_randtest,
                           as.krandtest(sim, obs,
                                        alter = c("two-sided", "two-sided",
                                                  "two-sided"),
...
for (name in names(results)) {
  write_csv(data.frame(results[name]), 
            "Data//Intermediate//amova_results.csv", append = TRUE,
            col_names = T)
}