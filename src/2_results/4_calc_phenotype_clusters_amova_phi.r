...
wheat_genind <- read_rds("Data\\Intermediate\\Adegenet\\wheat_genind.rds")
...
results <- tibble()
for (stratum in names(wheat_genind$strata)) {
  print(stratum)
  amova_result <- poppr.amova(
    wheat_genind, hier = str_c("~", stratum) %>% as.formula(), cutoff = 0.1,
