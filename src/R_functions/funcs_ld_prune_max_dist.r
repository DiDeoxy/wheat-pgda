...
find_windows <- function (chrom, max_dist, num_rows, genome_mean_dist) {
  windows <- vector()
  for (i in 1:num_rows) {
    j <- i
    # while j is less than num_rows, the difference between j and i is less
    # than the maximum number of snp allowed in the max_snps and the distance
    # between the snps is less than the max_dist allowed increment j
    while (j < num_rows && chrom$pos[j] - chrom$pos[i] < max_dist) {
      j <- j + 1
    }
    # j - i is the size of the max_snps for the ith snp
    windows <- c(windows, j)
  }
  # return the windows
  windows
...
adj_mat_maker <- function (chrom, max_dist, ld_mat, ld_floor, ld_ceiling,
  genome_mean_dist) {
    # for each i find j which is the downstream snp fulfilling the search
    # criteria of find_windows
    windows <- find_windows(chrom, max_dist, nrow(ld_mat),
      genome_mean_dist)
...
ld_prune <- function (wheat, maf, max_dist, ld_floor, ld_ceiling) {
  ...
  by(snp_data, snp_data$chrom, function (chrom) {
    ...
    temp <- adj_mat_maker(chrom, max_dist, ld_mat, ld_floor,
