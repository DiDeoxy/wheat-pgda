...
for (grouping in c("HS", "RW", "SW",
                   ...
                   "dend")[1:3]) {
  print(grouping)
  amovaResult <- poppr.amova(genind, hier = as.formula(paste0("~", grouping)), cutoff= 0.1, 
...
lapply(results, function (x) { write.table(data.frame(x), 'amova_results.csv', append = T, sep = ",") })
