...
sig_markers <- sig_markers[-which(sig_markers$chrom == "5A" & sig_markers$pos > 547000000 & sig_markers$pos < 550000000),]
sig_markers <- sig_markers[-which(sig_markers$chrom == "5B" & sig_markers$pos > 526000000 & sig_markers$pos < 529000000),]
sig_markers <- sig_markers[-which(sig_markers$chrom == "1A" & sig_markers$pos > 12000000 & sig_markers$pos < 14000000),]
sig_markers <- sig_markers[-which(sig_markers$chrom == "1B" & sig_markers$pos > 15000000 & sig_markers$pos < 18000000),]
sig_markers <- sig_markers[-which(sig_markers$chrom == "1D" & sig_markers$pos > 10000000 & sig_markers$pos < 12000000),]
sig_ranges <- GRanges(
    sig_markers$chrom,
    IRanges(sig_markers$pos - 500000, sig_markers$pos + 500000)
)

# gene_ranges <- makeTxDbFromGFF("/workspace/repos/Triticum_aestivum.IWGSC.42.gtf") %>% GenomicFeatures::genes()
