...
import::from(dplyr, "arrange", "distinct", "mutate", "rename", "rowwise", "select")
...
import::from(
  ...
  "geom_point",
  "geom_smooth",
  "guide_legend", "labs", "scale_colour_manual", "scale_fill_manual",
  "scale_shape_manual",
  "scale_size_manual",
...
snp_data <- snp_data %>%
  ...
  spread(type, jdb) %>%
  rename(`CHRS MJAF` = chrs, `CHRW MJAF` = chrw, `CSWS MJAF` = csws) %>% 
  gather(value_type, values, -c(id, chrom, pos_mb)) %>%
  arrange(chrom, pos_mb)

# head(snp_data)
# snp_data$value_type %>% unique()
...
lapply(chroms_data, function(chrom_data) {
  ...
  p1 <- chrom_data %>%
    ...
    geom_point(
      aes(
        colour = value_type, shape = value_type,
        size = value_type, fill = value_type
      )
    ) +
