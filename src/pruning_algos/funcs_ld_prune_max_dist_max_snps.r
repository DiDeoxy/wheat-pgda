...
parse_ld <- function (chrom, adj_mat, kept, i, j, pruned) {
  # create the cliques for the window determined by i to j (the max position
  # in the window determined by windows[i])
  ...
  # if there are no cliques and no snps in the window have been kept or are
...
cliques_frame_builder <- function (i, j, chrom, adj_mat) {
  ...
  # window)
  sub_mat <- matrix(adj_mat[i:j, i:j], nrow = (j - i + 1), ncol = (j - i + 1),
...
find_windows <- function (chrom, max_snps, max_dist, num_rows,
  genome_mean_dist) {
    windows <- vector()
    for (i in 1:num_rows) {
      j <- i
      # while j is less than num_rows, the difference between j and i is less
      # than the maximum number of snp allowed in the window and the distance
      # between the snps is less than the max_dist allowed increment j
      while (j < num_rows && j - i + 1 <= max_snps &&
        chrom$pos[j] - chrom$pos[i] < max_dist) {
          j <- j + 1
        }
      # j - i is the size of the window for the ith snp
      windows <- c(windows, j)
    }
    # return the windows
    windows
...
adj_mat_maker <- function (chrom, max_snps, max_dist, ld_mat, ld_floor, 
  ld_ceiling, genome_mean_dist) {
    # for each i find j which is the downstream snp fulfilling the search
    # criteria of find_windows
    windows <- find_windows(chrom, max_snps, max_dist, nrow(ld_mat),
      genome_mean_dist)
...
ld_prune <- function (wheat, maf, max_snps, max_dist, ld_floor, ld_ceiling) {
  ...
  snp_maf <- snpgdsSelectSNP(wheat, autosome.only = FALSE, maf = maf)
  ...
  by(snp_data, snp_data$chrom, function (chrom) {
    ...
    temp <- adj_mat_maker(chrom, max_snps, max_dist, ld_mat, ld_floor,
      ld_ceiling, genome_mean_dist)
