...
pop.code <- vector()
pop.code[match(labels(neighbour), sample.id)] <- c(rep("Hard Red Spring", 200), rep("Prairie Spring Red", 34),
                                                   rep("Out", 1))
pop.code <- factor(pop.code)
sample.compare <- c(which(pop.code == "Hard Red Spring"), which(pop.code == "Prairie Spring Red"))
...
## plotting
png("C:\\Users\\Max_H\\OneDrive - University of Guelph\\Pedagogy\\PGDA\\Analysis\\Figures\\loci\\HRS_fst_HRS_vs_PSR.png", 
...
title("Fst by Chromosome Between HRS and PSR", outer = T, cex.main = 1.5)
dev.off()
