...
## loading the data
load("C:\\Users\\Max_H\\OneDrive - University of Guelph\\Pedagogy\\PGDA\\Data\\bhc_dend.Rdata")
...
png("C:\\Users\\Max_H\\OneDrive - University of Guelph\\Pedagogy\\PGDA\\Figures\\dend\\bhc_dend_compact.png", 
...
circos.track(ylim = c(0, 1), panel.fun = function(x, y) {
  circos.text(1:359, rep(0, 359), labels(bhc.dend),
...
circos.track(ylim = c(0, 1), panel.fun = function(x, y) {
  circos.rect(0.05:358.05, rep(0,359), 0.95:358.95, rep(1,359), border = colors()[468], lwd = 0.6,
              col = sapply(as.numeric(as.character(pop.code))[match(labels(bhc.dend), sample.id)], function(x) { return(colours[x]) } ))
...
circos.track(ylim = c(0, 1), panel.fun = function(x, y) {
  circos.rect(0.05:358.05, rep(0,359), 0.95:358.95, rep(1,359), border = colors()[142], lwd = 0.6,
              col = sapply(as.numeric(pop.code)[match(labels(bhc.dend), sample.id)], function(x) { return(colours[x]) } ))
}, track.height = 0.04, bg.border = NA)
...
palette(colours)
dend_colour <- color_branches(bhc.dend, k = 7, col = colours)
