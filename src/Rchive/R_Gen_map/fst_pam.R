...
fst <- snpgdsSlidingWindow(wheat.data, FUN = "snpgdsFst", unit = "locus", winsize = 1, shift = 1, 
                           snp.id = snp.set.id, population = factor(pamk$pamobject$clustering), 
                           method ="W&H02", win.start = 0)
...
## plotting
png("C:\\Users\\Max_H\\OneDrive - University of Guelph\\Pedagogy\\PGDA\\Figures\\loci\\fst\\fst_pam.png", 
...
for (i in c(seq(3,75,12),seq(7,79,12),seq(11,83,12))) {
  fst_chr <- vector()
  for (j in 1:length(fst[[i]])) {
    fst_chr <- c(fst_chr, fst[[i]][[j]][[1]])
  }
  if (i %in% c(3,7,11)) {
    plot(snp.pos.2[[count]][1:length(fst_chr)], fst_chr, ylim = ylim, cex.axis = cex,
         pch = pch, col = colours[1 + count %% 7])
    abline(h = signif, col = "blue")
    abline(h = signif2, col = "red")
  } else {
    plot(snp.pos.2[[count]][1:length(fst_chr)], fst_chr, ylim = ylim, cex.axis = cex,
         pch = pch, col = colours[1 + count %% 7], yaxt = "n")
    abline(h = signif, col = "blue")
    abline(h = signif2, col = "red")
  }
  mtext(3, text = paste(labels[count + 1], ": ", length(fst_chr)),
        line = -1.25, cex = cex)
  count <- count + 1
  pos <- pos + length(fst_chr)
}
title(xlab = "Marker by Position in cM", outer = T, cex.lab = 1.5, line = 0.5)
title(ylab = "Wright's Fst Value", outer = T, cex.lab = 1.5)
title("Manhattan Plots of Fst Between PAM Groups", outer = T, cex.main = 1.5)
dev.off()
