...
pop.code <- as.factor(cutree(dend, k = 7))
...
fst <- snpgdsSlidingWindow(wheat.data, FUN = "snpgdsFst", unit = "locus", winsize = 1, shift = 1, 
                           snp.id = snp.set.id, population = pop.code, 
...
for (i in c(seq(3, 75, 12),seq(7, 79, 12),seq(11, 83, 12))) {
  fst_all <- c(fst_all, unlist(fst[[i]]))
}
...
## plotting
png("C:\\Users\\Max_H\\OneDrive - University of Guelph\\Pedagogy\\PGDA\\Figures\\loci\\fst\\fst_full.png", 
    family="Times New Roman", width = 6, height = 6, pointsize = 10, units = "in", res = 150)
par(mfrow = c(3,7), oma = c(3,5,3,2), mar = c(3, 0, 0, 0))
count = 1
pos = 0
for (i in c(seq(3,75,12),seq(7,79,12),seq(11,83,12))) {
  fst_chr <- unlist(fst[[i]])
  if (i %in% c(3,7,11)) {
    plot(snp.pos.2[[count]][1:length(fst_chr)], fst_chr, ylim = ylim, cex.axis = cex, pch = pch, 
