...
png("Results\\pca\\full_pca_CCP.png", family = "Times New Roman",
    width = 6, height = 7, pointsize = 10, units = "in", res = 300)
lbls <- paste("PC", 1:4, "\n", format(pc.percent[1:4], digits = 2), "%", sep="")
pairs(pca$eigenvect[,1:3], col = coloursDBSCAN[bests],
      labels=lbls, pch = 19, cex = 0.8, 
      main = "First Three PCs Plotted Against Each Other\nWith Varieties Coloured by 6 CCP K-means Groups",
      oma = c(6,2,8,2)
      )
par(xpd = T)
legend("bottom", legend = groups,
       pch = 19, col = coloursDend, cex = 0.6, horiz = T)
dev.off()

# # 3D plot
# plot3d(pca$eigenvect[,1:3], col = coloursIcl[bests], type = "s", size = 1, xlab = "Eigenvector 1",
